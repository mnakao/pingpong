program pingpong_put
  ...
  do i = 1, iter
     ...
     do j = 1, loop
        if (me .eq. 1) then
           loc_buf(1:last_element(i):stride) = rmt_buf(1:last_element(i):stride)[2];
           sync all
           do while (loc_buf(1) .ne. 9)
              sync memory
           end do
           do while (loc_buf(last_element(i)) .ne. 9)
              sync memory
           end do
           loc_buf(1) = -1
           loc_buf(last_element(i)) = -1
           sync all
        else
           sync all
           loc_buf(1:last_element(i):stride) = rmt_buf(1:last_element(i):stride)[1];
           do while (loc_buf(1) .ne. 9)
              sync memory
           end do
           do while (loc_buf(last_element(i)) .ne. 9)
